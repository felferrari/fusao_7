...
label = np.load(os.path.join(prep_path, f'label_2020.npy'))
...
for test_image in range(n_imgs):
    ...
    max_cloud_map =  np.maximum(
        np.load(os.path.join(prep_path, f'cmap_2019_{test_image}.npy')),
        np.load(os.path.join(prep_path, f'cmap_2020_{test_image}.npy'))
    )
