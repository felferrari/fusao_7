...
from models.models import sm_resunet
from ops.ops import load_json
import os
from models.losses import WBCE
from ops.dataloader import get_train_val_dataset, data_augmentation, prep_data_sar
...
ds_train = ds_train.map(prep_data_sar, num_parallel_calls=AUTOTUNE)
ds_train = ds_train.shuffle(50*batch_size)
ds_train = ds_train.batch(batch_size)
ds_train = ds_train.prefetch(AUTOTUNE)

ds_val = ds_val.map(data_augmentation, num_parallel_calls=AUTOTUNE)
ds_val = ds_val.map(prep_data_sar, num_parallel_calls=AUTOTUNE)
...
for model_idx in range(n_train_models):
    model = sm_resunet(shape_sar, model_size, n_classes, reg_weight = None, name = exp_name)
