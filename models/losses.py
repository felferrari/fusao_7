...
class WFocal(Loss):
    def __init__(self, weights=1.0, gamma = 2, class_indexes = None, **kwargs):
        super(WFocal, self).__init__(**kwargs)
        self.weights = weights
        self.gamma = gamma
        #self.weights = tf.constant([weights])
        self.class_indexes = class_indexes
    ...
    def call(self, y_true, y_pred, **kwargs):
        ...
        y_pred = tf.clip_by_value(y_pred, K.epsilon(), 1.0 - K.epsilon())
        loss = y_true * tf.math.log(y_pred) * tf.math.pow(1 - y_pred, self.gamma) + (1-y_true) * tf.math.log(1-y_pred) * tf.math.pow(y_pred, self.gamma)
        loss = loss * weights 
        loss = - tf.math.reduce_mean(loss, -1)
        #keep = tf.math.argmax(y_true, axis=-1) != 2
        #loss = tf.reshape(loss, [-1])[tf.reshape(keep, [-1])]
